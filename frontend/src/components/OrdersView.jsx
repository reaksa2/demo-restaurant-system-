...
export default function OrdersView({ orders, onStatusChange }) {
  ...
  const tableGroups = useMemo(() => {
    ...
    return Object.values(groups).sort((a, b) => {
      const aLatest = Math.max(
        ...a.orders.map((o) => parseUtcDate(o.created_at).getTime()),
      );
      const bLatest = Math.max(
        ...b.orders.map((o) => parseUtcDate(o.created_at).getTime()),
      );
  ...
  if (orders.length === 0) {
    return (
      <p className="py-12 text-center text-sm text-slate">No orders yet.</p>
    );
  }
        ...
        >
          ...
          <div className="divide-y divide-sand">
            {group.orders
              ...
              .sort(
                (a, b) =>
                  parseUtcDate(a.created_at) - parseUtcDate(b.created_at),
              )
              .map((o) => (
                <div key={o.id} className="px-5 py-3">
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-xs text-slate">
                      {parseUtcDate(o.created_at).toLocaleTimeString([], {
                        hour: "numeric",
                        minute: "2-digit",
                      })}
        ...
        <div key={o.id} className="px-5 py-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <Badge tone={TELEGRAM_TONES[o.telegram_notified]}>
                {TELEGRAM_LABELS[o.telegram_notified]}
              </Badge>
            </div>
